...
% If the Supplicant is successfully authenticated, the switch port state changes to authorized. If the authentication fails, the port remains in the unauthorized state, but authentication can be retried. If the authentication server cannot be reached, authentication fails, and network access is not granted.
