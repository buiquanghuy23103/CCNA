...
To ensure that the switch has the lowest bridge priority value, use the following command.
    ...
    \end{verbatim}
The above command dynamically makes priority of the switch to be the lowest in the broadcast domain. By default, the priority for the root switch is \textbf{24576}. If an alternate root bridge is desired, use the following command. 
    \begin{verbatim}
    S1(config)# spanning-tree vlan 10 root secondary
    \end{verbatim}   
The above command ensures that the alternate switch becomes the root bridge if the primary root bridge fails. This command, by default, sets the priority to \textbf{28672}.
